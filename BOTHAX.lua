...
local ai_queue = {}
local delay_text = 3000
local spam_text = "Hello World"
local spam_start = false
...
add_button_with_icon|sosial|`5Social Portal|staticPurpleFrame|1366||
add_button_with_icon|cmd|`5List Command|staticPurpleFrame|3524||
add_button_with_icon|information|`cInfo|staticBlueFrame|14514||
add_buttob_with_icon|spin_mode|`1Spin Mode|staticBlueFrame|758||
add_button_with_icon|wmenu|`1Wrench Menu|staticBlueFrame|15418||
add_button_with_icon||END_LIST|noflags|0||
add_button_with_icon|dialogmenu|`1UI Color Menu|staticBlueFrame|12166||
add_button_with_iconn|prefixmenu|`1Prefix Menu|staticPurpleFrame|12170||
add_button_with_icon|skinmenu|`1Skin Menu|staticBlueFrame|14414||
add_button_with_icon|logs|`1Logs|staticBlueFrame|1436||
add_button_with_icon|spamm|`1Spam Menu|staticBlueFrame|16238||
add_button_with_icon||END_LIST|noflags|0||
add_quick_exit||
]]
...
set_border_color|]] .. BD_red .. [[,]] .. BD_green .. [[,]] .. BD_blue .. [[,]] .. BD_transparan .. [[|
set_bg_color|]] .. BG_red .. [[,]] .. BG_green .. [[,]] .. BG_blue .. [[,]] .. BG_transparan .. [[|
add_label_with_icon|big|`2AI `wLogs|left|1436|
add_spacer|small|
add_button|resetai|`4Reset|noflags|0|
]]..LogAI..[[
add_spacer|small|
add_quick_exit||
add_button|world_spin|Back|
...
add_button|log_ai|`b[`2AI Logs`b]|
add_spacer|small|
add_quick_exit||
...
function send_ai_logs(ai1, ai2)
    ...
    local requestBody = [[{
        "username": "[ Kazelua ] AI LOGS",
        "avatar_url": "https://cdn.discordapp.com/attachments/1384472540876902480/1384497376995053649/Nazuna_Hiwatashi.png",
        "embeds": [{
            "title": "<:gpt:1442090269767106580> AI LOGS",
            "description": "**Nama:** ]] .. pname .. [[\n**Ask :** ]] .. ask .. [[\n**Answer :** ]] .. ans .. [[\n🕒 Time: ]] .. jam .. [[ (]] .. tanggal .. [[)",
            "color": 16777215
        }]
    }]]
...
RunThread(function()
		if skin_blink then
			...
			for _, color in ipairs(colors) do
				if not skin_blink then break end
...
function clean_ai_text(t)
    ...
    cleaned = cleaned:gsub("\\u(%x%x%x%x)", function(hex)
...
RunThread(function()
    while true do
        if #ai_queue > 0 then
            local task = table.remove(ai_queue, 1)
            local pertanyaan = task.q
  ...
  "contents": [{
    "parts": [{
      "text": "]] .. pertanyaan .. [["
            ...
            local res = MakeRequest(URL, "POST", headers, body)
            local raw = res.content
            local text = raw:match('"text"%s*:%s*"(.-)"')
            local jawab = clean_ai_text(text or raw)

            logs("`2KazeluaAI :\n`c" .. jawab)
            LogAI = LogAI.."\nadd_label_with_icon|small|`w[`2"..os.date("%H:%M").."`w] [`2 AI `w] `2"..jawab.."|left|8|\n"
            send_ai_logs(pertanyaan, jawab)
        end

        Sleep(100) -- biar ga makan CPU
    end
end)
...
function spam()
naon = [[set_border_color|]] .. BD_red .. [[,]] .. BD_green .. [[,]] .. BD_blue .. [[,]] .. BD_transparan .. [[|
set_bg_color|]] .. BG_red .. [[,]] .. BG_green .. [[,]] .. BG_blue .. [[,]] .. BG_transparan .. [[|
set_default_color|`w
add_label_with_icon|big|`8Kazelua Text Spammer    |left|16238|
add_spacer|small|
add_textbox|`2Text Input :<CR>`wset your text to spam, example : `^Hello World|left|
add_text_input|spam_input||]]..spam_text..[[|32|
add_spacer|small|
add_textbox|`2Delay Spam (ms) :<CR>`wset your delay spam, example : `^3000|left|
add_text_input|spam_delay||]]..delay_text..[[|5|
add_spacer|small|
add_inner_image_label_button|start_spam|`2Start Spam|game/custom_tiles3.rttex|1.6|16|9|32|
add_spacer|small|
add_button|backmenu|Back|
end_dialog|cmd|   CLOSE    |
add_quick_exit|
...
add_text_input|spam_delay||]]..delay_text..[[|5|
add_spacer|small|
add_inner_image_label_button|stop_spam|`4Stop Spam|game/custom_tiles3.rttex|1.6|18|4|32|
add_spacer|small|
add_button|backmenu|Back|
end_dialog|cmd|   CLOSE    |
add_quick_exit|
...
if str:find("buttonClicked|log_ai") or str:find("/lai") then
lai()
return true
end
...
if str:find("buttonClicked|resetai") then
LogAI = ""
return true
end
-- misc
...
if str:find("buttonClicked|spamm") or str:find("/spams") then
if spam_start == true then
stopspam()
else
  spam()
...
-- ai
if str:find("/ai (.+)") then
    local tanya = str:match("/ai (.+)")
    
    table.insert(ai_queue, {
        q = tanya,
        player = removeColorAndSymbols(GetLocal().name)
    })

    logs("`6AI Sedang memproses pertanyaan mu`0")
    return true
end
...
-- Spammer 
if str:find("buttonClicked|start_spam") and str:find("spam_input|(.+)\nspam_delay|(%d+)") then
  spam_text,delay_text = str:match("spam_input|(.+)\nspam_delay|(%d+)")
  ov("`2Spam is Enable\n`2Text : `w"..spam_text.."\n`2Delay : `w"..delay_text.." `2ms")
  if spam_start == false then
    spam_start = true
  return true
end
end

if str:find("buttonClicked|stop_spam") then
  if spam_start == true then
    spam_start = false
    ov("`4Spam is stopped")
    end
...
add_smalltext|`9Command > `#/ai (question) `1Chat with ai, `wex : `^/ai halo|
...
add_spacer|small|
add_smalltext|`2[ + ] `9Added Commands: `2/ai (question)|
add_smalltext|`2[ + ] `9Added Spam Menu|
add_spacer|small|
